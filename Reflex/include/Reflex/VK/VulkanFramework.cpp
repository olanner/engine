	...
	bool				useDebugLayers)
{
	...
	VK_FALLTHROUGH(InitSurface(windowInfo));
	VK_FALLTHROUGH(InitSwapchain(windowInfo));
	VK_FALLTHROUGH(InitSwapchainImageViews());
	VK_FALLTHROUGH(InitRenderPasses());
	VK_FALLTHROUGH(InitViewport(windowInfo));
	VK_FALLTHROUGH(InitDepthImage(windowInfo));
	VK_FALLTHROUGH(InitFramebuffers(windowInfo));
...
VkResult
VulkanFramework::InitSwapchain(const WindowInfo& windowInfo)
{
	...
	swapInfo.imageExtent = {windowInfo.width, windowInfo.height};
...
VkResult
VulkanFramework::InitViewport(const WindowInfo& windowInfo)
{
	// VIEWPORT
	...
	myScissor.extent = {windowInfo.width, windowInfo.height};
...
VkResult
VulkanFramework::InitDepthImage(const WindowInfo& windowInfo)
{
	// IMAGE
	...
	imageInfo.extent.width = windowInfo.width;
	imageInfo.extent.height = windowInfo.height;
...
VkResult
VulkanFramework::InitFramebuffers(const WindowInfo& windowInfo)
{
	...
	for (auto& image : mySwapchainImageViews)
	{
		...
		fBufferInfo.width = windowInfo.width;
		fBufferInfo.height = windowInfo.height;
